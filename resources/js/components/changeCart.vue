    ...
    <section class="py-4 text-center mx-ax">
        <form action = "/updateCart" method="get">
            <dl class="contact7">
                ...
                <!--
                    <template v-if="previousQuantity === 1">
                        <option value="1" selected>１</option>
                    </template>
                    <template v-else>
                        <option value="1" selected>１</option>
                    </template>
                    <template v-if="previousQuantity === 2">
                        <option value="2" selected>２</option>
                    </template>
                    <template v-else>
                        <option value="2">２</option>
                    </template>
                    <template v-if="previousQuantity === 3">
                        <option value="3" selected>３</option>
                    </template>
                    <template v-else>
                        <option value="3">３</option>
                    </template>
                    <template v-if="previousQuantity === 4">
                        <option value="4" selected>４</option>
                    </template>
                    <template v-else>
                        <option value="4">４</option>
                    </template>
                    <template v-if="previousQuantity === 5">
            ...
            <input type = "text" name="id" :value= "this.$route.query.id" class="d-none">
            <input type = "text" name="product_id" :value= "this.$route.query.product_id" class="d-none">
            <input type = "text" name="name" :value= "this.$route.query.name" class="d-none">
            ...
            <p>合計請求額：
                ...
                <template v-if="selectValue === '2'" name="purchasePrice" value = "this.$route.query.price">{{this.$route.query.price * 2}}円</template>
                <template v-if="selectValue === '3'" name="purchasePrice" value = "this.$route.query.price">{{this.$route.query.price * 3}}円</template>
                <template v-if="selectValue === '4'" name="purchasePrice" value = "this.$route.query.price">{{this.$route.query.price * 4}}円</template>
                <template v-if="selectValue === '5'" name="purchasePrice" value = "this.$route.query.price">{{this.$route.query.price * 5}}円</template>
            </p>
            <button type="submit" class="btn btn-primary btn-hover d-inline-block mx-4" name="cart">カートに再度入れる</button>
...
<script>
    export default{
        ...
        data(){
            return{
                countPrice : '',
                selectValue : this.$route.query.cartQuantity
            }
        },
